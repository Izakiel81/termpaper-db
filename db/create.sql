...
CREATE TABLE Journal(
    ...
    journal_teacher INT NOT NULL REFERENCES Teacher(teacher_id) ON DELETE SET NULL,
...
CREATE TABLE Days(
    ...
    day_timetable INT NOT NULL REFERENCES Timetable(timetable_id) ON DELETE SET NULL
);
CREATE TABLE Class(
    class_name VARCHAR(10) PRIMARY KEY CHECK ( class_name ~ '^(?:[1-9]|1[0-2])-([А-ЩЬЮЯҐЄІЇ]|[а-щьюяґєії])$'),
    class_journal_id INT NOT NULL REFERENCES Journal(journal_id) ON DELETE SET NULL,
    class_mainTeacher INT NOT NULL UNIQUE REFERENCES Teacher(teacher_id) ON DELETE SET NULL
);
CREATE TABLE Timetable(
    timetable_id INT PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY,
    timetable_name VARCHAR(20) NOT NULL UNIQUE,
    timetable_class VARCHAR(10) NOT NULL UNIQUE REFERENCES Class(class_name) CHECK ( timetable_class ~ '^(?:[1-9]|1[0-2])-([А-ЩЬЮЯҐЄІЇ]|[а-щьюяґєії])$') ON DELETE SET NULL
);
...
CREATE TABLE Homework(
    ...
    homework_teacher INT NOT NULL REFERENCES Teacher(teacher_id) ON DELETE SET NULL,
...
CREATE TABLE Students(
    ...
    student_class VARCHAR(10) NOT NULL REFERENCES Class(class_name) ON DELETE SET NULL
);
