...
class JournalModel {
  ...
  static async create(teacherId, name, db = pool) {
    const query = `CALL proc_create_journal($1, $2)`;
    const values = [teacherId, name];

    try {
      await db.query(query, values);
      const selectQuery = `SELECT * FROM journal WHERE journal_teacher = $1 AND journal_name = $2 ORDER BY journal_id DESC LIMIT 1`;
  ...
  static async update(id, teacherId, name, db = pool) {
    const query = `CALL proc_update_journal($1, $2, $3)`;
    const values = [id, teacherId, name];

    try {
      await db.query(query, values);
      const selectQuery = `SELECT * FROM journal WHERE journal_id = $1`;
  ...
  static async delete(id, db = pool) {
    const query = `CALL proc_delete_journal($1)`;
    const values = [id];
