...
class StudentModel {
  ...
  static async recieveAttendanceReport(studentId) {
  ...
  static async recieveDayPlan(studentId, startDate, endDate) {
    const tryQueries = [
      {
        text: `SELECT * FROM student_day_plan($1::integer, $2::date, $3::date)`,
        values: [studentId, startDate, endDate],
      },
      {
        text: `SELECT * FROM student_day_plan($1::integer, $2::date)`,
        values: [studentId, startDate],
      },
      // try function that only requires student id
      {
        text: `SELECT * FROM student_day_plan($1::integer)`,
        values: [studentId],
      },
      // alternate naming
      {
        text: `SELECT * FROM get_student_day_plan($1::integer, $2::date, $3::date)`,
        values: [studentId, startDate, endDate],
      },
      {
        text: `SELECT * FROM get_student_day_plan($1::integer)`,
        values: [studentId],
      },
      // fallback to view/table by student id
      {
        text: `SELECT * FROM vw_student_day_plan WHERE student_id = $1 ORDER BY lesson_date DESC`,
        values: [studentId],
      },
      // very generic fallback: try a timetable join (may or may not exist)
      {
        text: `SELECT t.day AS day, t.time AS time, t.subject AS subject, t.teacher AS teacher, t.room AS room FROM student_timetable t WHERE t.student_id = $1 ORDER BY t.day, t.time`,
        values: [studentId],
      },
    ];
    ...
    // If nothing returned rows, return empty array
    return [];
  }

  // Procedures

  static async create(name, surname, patronym, phone, class_c, user_id) {
    const query = `CALL proc_create_student($1::character varying, $2::character varying, $3::character varying, $4::character varying, $5::integer, $6::character varying, NULL)`;
    const values = [name, surname, patronym, phone, user_id, class_c];
    try {
  ...
  static async update(
    ...
    const query = `CALL proc_update_student($1::integer, $2::character varying, $3::character varying, $4::character varying, $5::character varying, $6::integer, $7::character varying)`;
    const values = [id, name, surname, patronym, phone, user_id, class_c];
    try {
