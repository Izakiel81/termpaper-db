...
class UserRoleModel {
  static async findRolesByUserId(userId, db = pool) {
    const query = `SELECT role_id FROM vw_user_roles WHERE user_id = $1`;
    const values = [userId];
    try {
  ...
  static async getAllUserRoles(db = pool) {
    const query = `SELECT * FROM vw_user_roles`;
    try {
  ...
  static async assignRole(userId, roleId, db = pool) {
    ...
    try {
      ...
      if (error.code === "22003") {
        throw new Error(`User with ID ${userId} or Role with ID ${roleId} does not exist.`);
      }
      if (error.code === "23505") {
        throw new Error(`User with ID ${userId} already has Role with ID ${roleId}.`);
      }
  ...
  static async removeRoleFromUser(userId, roleId, db = pool) {
    ...
    try {
      ...
      if (error.code === "22003") {
        throw new Error(`User with ID ${userId} does not exist or Role with ID ${roleId} is not assigned to this user.`);
      }
