...
class HomeworkModule {
  ...
  static async create(name, teacher, lesson, duedate, desc, class_c, db = pool) {
    ...
    try {
      ...
      if (error.code === '23503') {
        throw new Error("Teacher, Lesson, or Class does not exist.");
      }
      if (error.code === '23514') {
        throw new Error("Homework description cannot be empty or Class format is invalid.");
      }
      if (error.code === '22007') {
        throw new Error("Due date cannot be in the past.");
      }
      if (error.code === '23502') {
        throw new Error("Due date cannot be NULL.");
      }
  ...
  static async update(id, name, teacher, lesson, duedate, desc, class_c, db = pool) {
    ...
    try {
      ...
      if (error.code === '22003') {
        throw new Error("Homework, Teacher, Lesson, or Class does not exist.");
      }
      if (error.code === '23514') {
        throw new Error("Homework description cannot be empty or Class format is invalid.");
      }
  ...
  static async delete(id, db = pool) {
    ...
    try {
      ...
      if (error.code === '22003') {
        throw new Error(`Homework with ID ${id} does not exist.`);
      }
