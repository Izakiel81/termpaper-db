...
class ParentModel {
  ...
  static async create(name, surname, patronym, phone = null, db = pool) {
    ...
    try {
      ...
      if (newParent.rows && newParent.rows.length > 0) {
        return {
          id: newParent.rows[0].new_parent_id,
          generated_password: newParent.rows[0].generated_password
        };
      }
      return null;
    } catch (error) {
      ...
      if (error.code === "23514") {
        throw new Error(`Required parent fields cannot be empty.`);
      }
      throw new Error(
  ...
  static async update(id, name, surname, patronym, phone, db = pool) {
    ...
    try {
      ...
      if (error.code === "22003") {
        throw new Error(`Parent with ID ${id} does not exist.`);
      }
      throw new Error(
  ...
  static async delete(id, db = pool) {
    const query = `CALL proc_delete_parent($1::integer)`;
    const values = [id];
