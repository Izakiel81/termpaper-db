...
class ParentModel {
  static async findAll() {
    ...
    try {
      const parents = await pool.query(query);
  ...
  static async findById(id) {
    ...
    try {
      const parent = await pool.query(query, values);
  ...
  static async create(name, surname, patronym, phone) {
    const query = `CALL proc_create_parent($1::character varying, $2::character varying, $3::character varying, $4::character varying, NULL)`;
    const values = [name, surname, patronym, phone];

    try {
      const newParent = await pool.query(query, values);
  ...
  static async update(id, name, surname, patronym, phone) {
    const query = `CALL proc_update_parent($1::integer, $2::character varying, $3::character varying, $4::character varying, $5::character varying, NULL)`;
