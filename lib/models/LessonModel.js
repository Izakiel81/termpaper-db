...
class LessonModule {
  static async findAll(db = pool) {
    const query = `SELECT * FROM vws_lessons`;
    try {
  ...
  static async findById(id, db = pool) {
    const query = `SELECT * FROM vws_lessons WHERE lesson_id=$1`;
    const values = [id];
  ...
  static async findByName(name, db = pool) {
    const query = `SELECT * FROM vws_lessons WHERE lesson_name=$1`;
    const values = [name];
    try {
      const lessons = await db.query(query, values);
      if (lessons.rows && lessons.rows.length > 0) {
  ...
  static async create(
    ...
    try {
      ...
      if (error.code === "22003") {
        throw new Error(`Class, Subject, Material, or Teacher does not exist.`);
      }
      throw new Error(
  ...
  static async update(
    ...
    try {
      ...
      if (error.code === "22003") {
        throw new Error(`Lesson, Teacher, Class, or Subject does not exist.`);
      }
      if (error.code === "23514") {
