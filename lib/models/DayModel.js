...
class DayModule {
  ...
  static async findById(id) {
    ...
    try {
      const days = await pool.query(query, values);
      if (days.rows && days.rows.length > 0) {
        return days.rows[0];
      }
      return null;
  ...
  static async create(p_day_name, p_day_time, p_day_weekday) {
    ...
    try {
      const newDay = await pool.query(query, values);
  ...
  static async update(p_id, p_time, p_weekday) {
