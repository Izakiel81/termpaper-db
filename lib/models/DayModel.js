...
class DayModule {
  ...
  static async create(p_subjectId, p_timetableId, p_day_time, p_day_weekday, db = pool) {
    ...
    try {
      ...
      console.error(
        `Error in create: Could not create new day entry.`,
        error,
      );
      if (error.code === '22003') {
        throw new Error(
          `Timetable or Subject does not exist.`,
        );
      }
      if (error.code === '23502') {
        throw new Error(
          `Day time cannot be NULL.`,
        );
      }
      if (error.code === '23514') {
        throw new Error(
  ...
  static async update(p_id, p_subjectId, p_timetableId, p_time, p_weekday, db = pool) {
    ...
    try {
      ...
      if (error.code === '22003') {
        throw new Error(
          `Day, Timetable, or Subject does not exist.`,
        );
      }
      if (error.code === '23514') {
        throw new Error(
  ...
  static async delete(p_id, db = pool) {
    ...
    try {
      ...
      if (error.code === '22003') {
        throw new Error(
          `Day with ID ${p_id} does not exist.`,
        );
      }
      throw new Error(
