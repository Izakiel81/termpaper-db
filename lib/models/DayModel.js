...
class DayModule {
  ...
  static async update(p_id, p_name, p_time, p_weekday, db = pool) {
    const query = `CALL proc_update_day($1::integer, $2::character varying $3::time without time zone, $4::character varying)`;
    const values = [p_id, p_name, p_time, p_weekday];
    try {
