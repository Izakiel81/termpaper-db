...
class RolesModel {
  ...
  static async create(name, desc = null, db = pool) {
    const query = `CALL proc_create_role($1, $2)`;
    const values = [name, desc];

    try {
      await db.query(query, values);
      const selectQuery = `SELECT * FROM roles WHERE role_name = $1`;
  ...
  static async update(id, name, desc = null, db = pool) {
    const query = `CALL proc_update_role($1, $2, $3)`;
    const values = [id, name, desc];

    try {
      await db.query(query, values);
      const selectQuery = `SELECT * FROM roles WHERE role_id = $1`;
  ...
  static async delete(id, db = pool) {
    const query = `CALL proc_delete_role($1)`;
    const values = [id];
