import pool from "../db.js";
class TeacherModel {
  static async findAll() {
    const query = `SELECT * from teacher`;

    try {
      const teachers = await pool.query(query);
      if (teachers.rows && teachers.rows.length > 0) {
        return teachers.rows;
      }
      return null;
    } catch (error) {
      console.error(`Database error finding teachers`, error);
      throw new Error(
        "Could not retrieve teachers data due to a database error.",
      );
    }
  }
  static async findById(id) {
    const query = `SELECT * FROM teacher ($1::integer)`;
    const values = [id];

    try {
      const teacher = await pool.query(query, values);
      if (teacher.rows && teacher.rows.length > 0) {
        return teacher.rows[0];
      }
      return null;
    } catch (error) {
      console.error(`Database error finding teacher`, error);
      throw new Error(
        "Could not retrieve teacher data due to a database error.",
      );
    }
  }

  // Views

  static async withClasses() {
    const query = `SELECT * from vw_teachers_with_classes`;

    try {
      const teachers = await pool.query(query);
      if (teachers.rows && teachers.rows.length > 0) {
        return teachers.rows;
      }
      return null;
    } catch (error) {
      console.error(`Database error finding teachers`, error);
      throw new Error(
        "Could not retrieve teachers data due to a database error.",
      );
    }
  }

  // Functions

  static async recieveSalary(teacherId, fromDate, toDate) {
    const query = `SELECT * FROM get_teacher_salary($1::integer, $2::date, $3::date)`;
    const values = [teacherId, fromDate, toDate];

    try {
      const result = await pool.query(query, values);
      if (result.rows && result.rows.length > 0) {
        return result.rows;
      }
      return null;
    } catch (error) {
      console.error(`Database error finding data:`, error);
      throw new Error("Could not retrieve data due to a database error.");
    }
  }

  // Procedures

  static async create(name, surname, patronym, phone, user_id = null) {
    const query = `CALL proc_create_teacher($1::character varying, $2::character varying, $3::character varying, $4::character varying, $5::integer, NULL)`;
    const values = [name, surname, patronym, phone, user_id];

    try {
      const newTeacher = await pool.query(query, values);
      if (newTeacher.rows && newTeacher.rows.length > 0) {
        return newTeacher.rows[0].new_teacher_id;
      }
      return null;
    } catch (error) {
      console.error(
        `Database error creating the teacher\t ${name}\t ${surname}\t ${patronym}\t ${phone}\t ${user_id}\t:`,
        error,
      );
      throw new Error(
        "Could not creating the teacher data due to a database error.",
      );
    }
  }

  static async update(id, name, surname, patronym, phone, user_id = null) {
    const query = `CALL proc_update_teacher($1::integer, $2::character varying, $3::character varying, $4::character varying, $5::character varying, $6::integer)`;
    const values = [id, name, surname, patronym, phone, user_id];

    try {
      await pool.query(query, values);
    } catch (error) {
      console.error(
        `Database error updating the teacher\t ${id}\t ${name}\t ${surname}\t ${patronym}\t ${phone}\t ${user_id}\t:`,
        error,
      );
      throw new Error(
        "Could not update the teacher data due to a database error.",
      );
    }
  }

  static async delete(id) {
    const query = `DELETE FROM teacher WHERE teacher_id=$1`;
    const values = [id];

    try {
      await pool.query(query, values);
    } catch (error) {
      console.error(`Database error deleting the teacher ${id}\t:`, error);
      throw new Error(
        "Could not delete the teacher data due to a database error.",
      );
    }
  }
}
export default TeacherModel;
