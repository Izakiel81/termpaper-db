...
class StudentDataModule {
  static async findAll() {
    ...
    try {
      const studentData = await pool.query(query);
  ...
  static async findById(id) {
    ...
    try {
      const studentData = await pool.query(query, values);
      if (studentData.rows && studentData.rows.length > 0) {
  ...
  static async create(
    ...
    try {
      const newStudentData = await pool.query(query, values);
  ...
  static async findMarks7d(studentId) {
  ...
  static async findMarks(studentId, from, to) {
