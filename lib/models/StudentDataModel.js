...
class StudentDataModule {
  static async findAll(db = pool) {
    const query = `SELECT * FROM vws_student_data`;
    try {
  ...
  static async findById(id, db = pool) {
    const query = `SELECT * FROM vws_student_data WHERE studentdata_id=$1`;
    const values = [id];
  ...
  static async create(
    ...
    try {
      ...
      if (error.code === "23503") {
        throw new Error(`Journal, Student, or Lesson does not exist.`);
      }
      if (error.code === "22003") {
        throw new Error(`Mark is out of range (1-12).`);
      }
      throw new Error(
  ...
  static async update(
    ...
    try {
      ...
      if (error.code === "22003") {
        throw new Error(`StudentData, Journal, Student, or Lesson does not exist, or Mark is out of range.`);
      }
      throw new Error(
