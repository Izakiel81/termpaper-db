...
class MaterialModule {
  ...
  static async findById(id) {
    ...
    try {
      const materials = await pool.query(query, values);
      if (materials.rows && materials.rows.length > 0) {
  ...
  static async update(p_id, p_name, p_desc, p_link) {
